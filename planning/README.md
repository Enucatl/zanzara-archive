...
Implement one bounded issue, run its required checks, open a PR with evidence and mark In review. Completion requires merged code and satisfied acceptance, or recorded human action for Operator tasks. At each phase boundary present `$review-phase <parent-issue-number>` for invocation under GPT 5.6-Sol. The skill reviews integrated behavior, verifies current evidence and creates deduplicated native remediation children; it does not fix them or release the phase. Every phase stays open until a current PASS and the user's `Release Pn at <reviewed-commit>` record.
