...
[corpus-20.json](corpus-20.json) fixes exactly 20 relative filenames from 2026-07-01 through 2026-09-10, with actual SHA-256, duration in milliseconds, codec, channel count and sample rate. The golden episode is `260910-lazanzara.opus`. P0-02 imports and verifies this manifest; it never selects the latest files again. Missing or changed sources stop dependent work. Archive-wide counts and durations in the original plan are estimates, not measured capacity.

Mount `/export/scratch/archive/zanzara` read-only at `/archive`. Resolve manifest relative filenames beneath that root, reject traversal and escaping symlinks, and validate checksums before processing. Never alter canonical Opus sources. FFmpeg decodes locally into temporary model-specific PCM, normally 16 kHz mono. Record the decoder version, resampling/channel policy, original duration and transform hash. A model needing another format gets an explicit transform fingerprint. Preserve the time origin; silence removal must not shift persisted timestamps.
