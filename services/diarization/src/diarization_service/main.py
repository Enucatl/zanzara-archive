...
def _native_activity(
    ...
    for index, raw_count in enumerate(data[:, 0].tolist()):
        ...
        frame_end_ms = to_ms(float(start_s) + index * float(step_s) + float(duration_s))
...
def _diarize(
    audio: np.ndarray, sample_rate: int, *, source_sha256: str = "pending"
...
def diarize_request(request: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    standard, exclusive, native = _diarize(audio, sample_rate, source_sha256=source_sha256)
